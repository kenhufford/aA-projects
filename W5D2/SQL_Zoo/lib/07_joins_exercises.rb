...
def andrews_films_and_leads
  ...
  FROM
    ...
    WHERE actors.name = 'Julie Andrews') AS JA_movies
  JOIN movies ON movies.id = JA_movies.movie_id
  JOIN castings ON castings.movie_id = JA_movies.movie_id
  JOIN actors ON castings.actor_id = actors.id
  WHERE ord = 1
  SQL
end
...
#   ON castings.movie_id = JA_movies.movie_id
#   INNER JOIN movies
#   ON movies.id = castings.movie_id
#   INNER JOIN actors
#   ON actors.id = castings.actor_id
...
def colleagues_of_garfunkel
  ...
  FROM
    ...
    FROM castings
    JOIN actors ON castings.actor_id = actors.id
    WHERE name = 'Art Garfunkel') AS garfunkel_movies
  JOIN castings ON garfunkel_movies.movie_id = castings.movie_id
  JOIN actors ON castings.actor_id = actors.id
